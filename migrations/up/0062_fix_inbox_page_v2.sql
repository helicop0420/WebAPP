...
CREATE OR REPLACE VIEW public.inbox_page_view_v2
    ...
    (EXISTS ( SELECT deal_interest.id
           FROM conversation_to_user_associations ctuas
             JOIN deal_interest ON ctuas.user_id = deal_interest.user_id
          WHERE deal_interest.deal_id = deal.id AND ctuas.conversation_id = conv.id AND deal_interest.user_id <> user_profile.user_id)) AS expressed_interest,
